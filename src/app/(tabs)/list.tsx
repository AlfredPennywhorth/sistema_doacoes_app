...
export default function ListScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Note: Our databaseService.js getDonations only takes category for now.
      // We should filter warehouses client-side or update the service.
      // For now, let's filter client-side as the volume is low.
      // Now we include reserved items to show visibility
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          >
            ...
            <Text style={styles.dashboardBtnText}>Painel</Text>
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  dashboardBtnText: { color: "#003366", fontSize: 12, fontWeight: "bold" },
  filterBar: {
    flexDirection: "row",
    gap: 10,
...
const CATEGORIES = ['TODOS', 'ALIMENTOS', 'ITENS HOSPITALARES', 'LINHA BRANCA', 'MÓVEIS', 'ÓRGÃO ELETRÔNICO', 'VESTUÁRIO', 'OUTROS'];
        ...
        <SafeAreaView style={styles.safeArea} edges={['top']}>
            <View style={styles.container}>
                ...
                <View style={styles.filterBar}>
                    ...
                    <TouchableOpacity style={styles.filterBtn} onPress={() => setShowWarehouseModal(true)}>
                        <MaterialIcons name="warehouse" size={18} color="#003366" />
                        <Text style={styles.filterText} numberOfLines={1}>
                            {selectedWarehouse === 'PRIVATE' ? 'PARTICULAR' : warehouses.find(w => w.id === selectedWarehouse)?.name || 'LOCAL'}
                        </Text>
                        <MaterialIcons name="arrow-drop-down" size={20} color="#003366" />
                    </TouchableOpacity>
                </View>
                ...
                {loading && !refreshing ? (
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            ...
                            >
                                <View style={styles.cardHeader}>
                                    <View style={[styles.categoryBadge, { backgroundColor: '#003366' }]}>
                                        <Text style={styles.categoryText}>{item.category}</Text>
                                    </View>
                                    {item.is_urgent && (
                                        <View style={styles.urgentBadge}>
                                ...
                                <Text style={styles.cardTitle}>{item.title}</Text>
                                <Text style={styles.cardDesc} numberOfLines={2}>{item.description ?? 'Sem descrição'}</Text>

                                <View style={styles.cardFooter}>
                                    <View style={styles.locationContainer}>
                                        <MaterialIcons name={item.warehouse ? "inventory" : "person-pin"} size={16} color="#64748b" />
                                        <Text style={styles.locationText}>
                    ...
                    <View style={styles.modalContent}>
                        ...
                        {CATEGORIES.map((cat) => (
                            <TouchableOpacity key={cat} style={styles.modalOption}
                                ...
                                <Text style={[styles.modalOptionText, selectedCategory === cat && styles.modalOptionActive]}>{cat}</Text>
                                {selectedCategory === cat && <MaterialIcons name="check" size={18} color="#003366" />}
                            </TouchableOpacity>
                        ))}
                    </View>
                </TouchableOpacity>
                ...
                <TouchableOpacity style={styles.modalOverlay} onPress={() => setShowWarehouseModal(false)} activeOpacity={1}>
                    <View style={styles.modalContent}>
                        ...
                        <TouchableOpacity style={styles.modalOption}
                            onPress={() => { setSelectedWarehouse(null); setShowWarehouseModal(false); }}>
                            <Text style={[styles.modalOptionText, !selectedWarehouse && styles.modalOptionActive]}>TODOS OS LOCAIS</Text>
                            {!selectedWarehouse && <MaterialIcons name="check" size={18} color="#003366" />}
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.modalOption}
                            onPress={() => { setSelectedWarehouse('PRIVATE'); setShowWarehouseModal(false); }}>
                            <Text style={[styles.modalOptionText, selectedWarehouse === 'PRIVATE' && styles.modalOptionActive]}>PARTICULARES</Text>
                            {selectedWarehouse === 'PRIVATE' && <MaterialIcons name="check" size={18} color="#003366" />}
                        </TouchableOpacity>
                        {warehouses.map((w) => (
                            <TouchableOpacity key={w.id} style={styles.modalOption}
                                onPress={() => { setSelectedWarehouse(w.id); setShowWarehouseModal(false); }}>
                                <Text style={[styles.modalOptionText, selectedWarehouse === w.id && styles.modalOptionActive]}>{w.name}</Text>
                                {selectedWarehouse === w.id && <MaterialIcons name="check" size={18} color="#003366" />}
                            </TouchableOpacity>
                        ))}
                    </View>
                </TouchableOpacity>
...
const styles = StyleSheet.create({
    safeArea: { flex: 1, backgroundColor: '#003366' },
    container: { flex: 1, backgroundColor: '#f8fafc' },
    header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 20, paddingBottom: 15, backgroundColor: '#003366' },
    headerTitle: { fontSize: 24, fontWeight: 'bold', color: '#fff' },
    headerSubtitle: { fontSize: 13, color: 'rgba(255,255,255,0.7)', marginTop: 2 },
    dashboardBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.1)', alignItems: 'center', justifyContent: 'center' },
    filterBar: { flexDirection: 'row', gap: 10, paddingHorizontal: 20, marginVertical: 15 },
    filterBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', paddingHorizontal: 12, paddingVertical: 10, borderRadius: 12, borderWidth: 1, borderColor: '#e2e8f0', gap: 6, justifyContent: 'center' },
    ...
    categoryBadge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
    categoryText: { color: '#fff', fontSize: 9, fontWeight: 'bold' },
    urgentBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#dc2626', paddingHorizontal: 6, paddingVertical: 3, borderRadius: 6, gap: 2 },
    urgentText: { color: '#fff', fontSize: 9, fontWeight: 'bold' },
    reservedBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#64748b', paddingHorizontal: 6, paddingVertical: 3, borderRadius: 6, gap: 2 },
    reservedText: { color: '#fff', fontSize: 9, fontWeight: 'bold' },
    ...
    centered: { flex: 1, justifyContent: 'center', alignItems: 'center' },
    emptyText: { marginTop: 12, color: '#64748b', fontSize: 14 },
    retryBtn: { marginTop: 16, backgroundColor: '#003366', paddingHorizontal: 24, paddingVertical: 10, borderRadius: 12 },
    retryText: { color: '#fff', fontWeight: 'bold' },
    fab: { position: 'absolute', bottom: 20, right: 20, width: 56, height: 56, borderRadius: 28, backgroundColor: '#003366', alignItems: 'center', justifyContent: 'center', elevation: 5, shadowColor: '#003366', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 5 },
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' },
    modalContent: { backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 },
    modalTitle: { fontSize: 18, fontWeight: 'bold', color: '#003366', marginBottom: 16 },
    modalOption: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
